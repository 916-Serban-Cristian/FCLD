...
public class Scanner {
    ...
    private SymbolTable<String> symbolTable, constantsTable;
    private List<Map.Entry<String, Integer>> pif;
    private int index;
    ...
    private boolean stringConstant() throws ScannerException {
        var strRegex = Pattern.compile("^\"([a-zA-z0-9_ ]*)\"");
    ...
    private boolean intConstant() {
        ...
        if (matcher.find()) {
            ...
            if (pif.size() > 0) {
                ...
                if ((token.charAt(0) == '+' || token.charAt(0) == '-') && (pif_last == 0 || pif_last == -1 || pif_last == -2)) {
                    return false;
                }
            }
    ...
    private boolean identifier() {
        ...
        if (matcher.find()) {
            var token = matcher.group(1);
            pif.add(new AbstractMap.SimpleEntry<>("id", symbolTable.pos(token)));
            index += matcher.end();
            return true;
        }
        return false;
